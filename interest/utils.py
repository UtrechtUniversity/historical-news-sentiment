...
# Logging
def setup_logging(log_file: str, level: int = logging.INFO, format: str = "%(asctime)s - %(levelname)s - %(message)s"):
    """
    ...
    Args:
        ...
        format (str, optional): Log message format. Defaults to a standard timestamped format.
    """
...
def prepare_data(
    ...
    Args:
        ...
        binary_labels (bool, optional): If True, converts labels to binary. If False, retains original multi-labels.
    ...
    # Splitting the data
    train_texts, train_labels, val_texts, val_labels, test_texts, test_labels = loader.split_data(
...
# - Ensure `TextPreprocessor` and `CSVDataLoader` adhere to required input-output standards.
