...
def hyperparameter_optimization(classifiers, param_grid, X_train, y_train):
    """
    Perform hyperparameter optimization for a set of classifiers using
    GridSearchCV.

    Args:
        classifiers (dict): A dictionary with classifier names as keys
        and classifier objects as values.
        param_grid (dict): A dictionary with classifier names as keys
        and their parameter grids as values.
        X_train (sparse matrix): Training data features.
        y_train (array-like): Training data labels.

    Returns:
        dict: Results of the optimization containing best parameters and
    ...
    for clf_name, clf in classifiers.items():
        ...
        grid_search = GridSearchCV(clf,
                                   param_grid[clf_name],
                                   cv=5, scoring={'f1_weighted': 'f1_weighted', 'accuracy': 'accuracy'},  # noqa:E501
...
def run_optimization_pipeline(train_data, train_labels, binary_labels):
    ...
    classifiers = {
        ...
        "Support Vector Machine": SVC(class_weight='balanced', kernel='rbf', random_state=42),  # noqa:E501
        "Logistic Regression": LogisticRegression(class_weight='balanced', max_iter=1000, random_state=42),  # noqa:E501
        "Random Forest": RandomForestClassifier(class_weight='balanced', random_state=42),  # noqa:E501
        "Naive Bayes": ComplementNB()  # noqa:E501
    }
    ...
    results = hyperparameter_optimization(classifiers,
                                          param_grid,
                                          X_train,
                                          train_labels)
